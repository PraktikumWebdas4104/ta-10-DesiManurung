<?php
	class model{
		...
		function __construct(){
			...
			$this->conn = mysqli_connect("localhost", "root","","db_ta10");
		}
		...
		function updateMhs($nim, $nama, $angkatan, $fakultas, $program,$genre,$wisata){
			$query = "UPDATE mahasiswa SET  nama='$nama', angkatan='$angkatan', fakultas='$fakultas' ,program='$program',genre='$genre',wisata='$wisata' where nim='$nim'";//query update nim, nama, angkatan, fakultas, prodi
			return $this->execute($query);
		}
		...
		function insertMhs($nim, $nama, $angkatan, $fakultas, $program,$genre,$wisata){
			$query = "INSERT into mahasiswa values ('$nim', '$nama', '$angkatan', '$fakultas', '$program','$genre','$wisata')";//query insert nim,nama, angkatan, fakultas, prodi
			return $this->execute($query);
		}
